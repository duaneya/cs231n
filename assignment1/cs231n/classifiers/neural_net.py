...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # classifier loss.                                                          #
        #############################################################################
        loss = np.mean(-scores[np.arange(N), y] + np.log(np.sum(np.exp(scores), axis=1))) + reg * np.sum(
            W1 * W1) + reg * np.sum(W2 * W2)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        ...
        prob /= N
        grads['W2'] = h.T.dot(prob)
        grads['W2'] += 2 * reg * W2
        grads['b2'] = np.sum(prob, axis=0, keepdims=True).reshape(-1, )
        dh = W2.dot(prob.T)
        dh = dh * (h.T > 0)
        grads['W1'] = (dh.dot(X)).T
        grads['W1'] += 2 * reg * W1
        grads['b1'] = np.sum(dh.T, axis=0, keepdims=True).reshape(-1, )
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
